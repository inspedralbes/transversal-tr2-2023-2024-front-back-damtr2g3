    ...
    for (let i = 0; i < dades.length; i++) {
        for (let j = 0; j < dades.length; j++) {
            if (dades[i].pregunta == dades[j].pregunta) {
                if (dades[j].correcta >= 0) {
                    dades[i].correcta += dades[j].correcta;
                }
                if (dades[j].incorrecta >= 0) {
                    dades[i].incorrecta += dades[j].incorrecta;
                }
                if(dades[j].temps >= 0){
                    ...
                    dades[i].temps += parseFloat(auxtime[0])
                }
                dades[i].temps = dades[i].temps / (dades[i].correcta + dades[i].incorrecta)
                dades.splice(j, 1);
            }
        }
    }
